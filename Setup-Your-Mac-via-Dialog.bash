...
#   - Updated by Scott Adamson: Oomnitza Inc 03082023
#
####################################################################################################
...
scriptLog="${4:-"/var/log/org.oomnitza.log"}"                        # Parameter 4: Script Log Location [ /var/log/org.churchofjesuschrist.log ] (i.e., Your organization's default location for client-side logs)
...
if [[ "${requiredMinimumBuild}" == "disabled" ]]; then
    ...
    if [[ "${osMajorVersion}" -ge 11 ]] ; then
        ...
        # When the current `osBuild` is older than `requiredMinimumBuild`; exit with error
            ...
            osascript -e 'display dialog "Please advise your TechDesk of the following error:\r\rExpected macOS Build '${requiredMinimumBuild}' (or newer), but found macOS '${osVersion}' ('${osBuild}').\r\r" with title "Setup Your Mac: Detected Outdated Operating System" buttons {"Open Software Update"} with icon caution'
    ...
    # The Mac is running an operating system older than macOS 11 Big Sur; exit with error
        ...
        osascript -e 'display dialog "Please advise the TechDesk of the following error:\r\rExpected macOS Build '${requiredMinimumBuild}' (or newer), but found macOS '${osVersion}' ('${osBuild}').\r\r" with title "Setup Your Mac: Detected Outdated Operating System" buttons {"Open Software Update"} with icon caution'
        updateScriptLog "Pre-flight Check: Executing /usr/bin/open '${outdatedOsAction}' …"
        su - "${loggedInUser}" -c "/usr/bin/open \"${outdatedOsAction}\""
        exit 1
...
function dialogCheck() {
    ...
    if [ ! -e "/Library/Application Support/Dialog/Dialog.app" ]; then
        ...
        if [[ "$expectedDialogTeamID" == "$teamID" ]]; then
            ...
            osascript -e 'display dialog "Please advise the TechDesk of the following error:\r\r• Dialog Team ID verification failed\r\r" with title "Setup Your Mac: Error" buttons {"Close"} with icon caution'
...
helpmessage="If you need assistance, please contact the TechDesk:  \n- **Email:** techdesk@oomnitza.com  \n- **Slack:** @scott.adamson  \n\n**Computer Information:** \n\n- **Operating System:**  ${macOSproductVersion} ($macOSbuildVersion)  \n- **Serial Number:** ${serialNumber}  \n- **Dialog:** ${dialogVersion}  \n- **Started:** ${timestamp}"
